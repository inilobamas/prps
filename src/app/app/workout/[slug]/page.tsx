...
export default function ProgramOverviewPage({ params }: ProgramOverviewProps) {
  ...
  useEffect(() => {
    ...
    const logs = getAllDayLogs().filter(log => log.programSlug === params.slug);
    setDayLogs(logs);
  }, [params.slug, router]);
                ...
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      <Button variant="outline" size="sm" asChild>
                        <Link href={`/app/workout/${params.slug}/${day.day}`}>
        ...
        <CardContent className="p-4">
          <div className="flex gap-2">
            ...
            <Button variant="outline" size="sm" onClick={() => {
              const data = getAllDayLogs().filter(log => log.programSlug === params.slug);
              const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
              const url = URL.createObjectURL(blob);
              const a = document.createElement('a');
              a.href = url;
              a.download = `${params.slug}-logs.json`;
              a.click();
            }}>
              Export Logs
            </Button>
